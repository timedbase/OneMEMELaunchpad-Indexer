...
export class DiscoverService {
  ...
  async trending(query: Record<string, string | undefined>) {
    ...
    return { ...paginated(rows.map(toCamel), count, page, limit), window };
  }
  ...
  async newTokens(query: Record<string, string | undefined>) {
    ...
    const [rows, [{ count }]] = await Promise.all([
      sql`SELECT * FROM token WHERE migrated = FALSE ${typeFilter} ORDER BY created_at_block::numeric DESC LIMIT ${limit} OFFSET ${offset}`,
